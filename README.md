...
- `web/.env.cloud`：云端联调，保留云端接入形态，但不包含任何真实凭证

私有凭证建议写入这些忽略文件：

- `web/.env.local.private`
- `web/.env.cloud.private`

兼容说明：

- 旧的私有 `web/.env` 仍然可继续使用
- `scripts/start-cloud-dev.sh` 在 `web/.env.cloud.private` 缺失时，会自动回退叠加现有的 `web/.env`
- 默认 `CONFIG_RESOLUTION_MODE=auto`
- 对象存储历史补迁移已不再依赖 Web 启动；需要时请手动运行 `scripts/sync_object_storage_history.py`

配置优先建议：

- 部署、密钥、数据库、对象存储、第三方接入放在 `web/.env.*`
...
- 本地开发脚本会加载 `web/.env.local`，并在存在时自动叠加 `web/.env.local.private`
...
- 云端联调脚本会加载 `web/.env.cloud`，并优先叠加 `web/.env.cloud.private`
- 若 `web/.env.cloud.private` 不存在，会回退叠加现有的私有 `web/.env`
- 如需显式指定自定义环境文件链路，可使用 `DEEPVISION_ENV_FILE=/path/a.env:/path/b.env uv run web/server.py`
...
- [scripts/start-local-dev.sh](scripts/start-local-dev.sh)：本地开发环境启动脚本（`web/.env.local` + 可选私有覆盖）
- [scripts/start-cloud-dev.sh](scripts/start-cloud-dev.sh)：云端联调环境启动脚本（`web/.env.cloud` + 私有覆盖 / 兼容旧 `web/.env`）
...
```bash
python3 scripts/sync_object_storage_history.py --env-file web/.env.cloud --env-file web/.env.cloud.private
