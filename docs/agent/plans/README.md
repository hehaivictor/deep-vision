...
- 用户只给了 1 到 4 句需求，需要先扩成结构化计划，再进入 task workflow
...
- Planner artifact 只关注目标、范围、完成定义、风险和推荐执行顺序，不在此阶段承诺底层实现细节
- Planner artifact 现在会与 Sprint Contract 一起进入 `progress.md`、`failure-summary.md` 和 `handoff.json`，便于下一位直接回看计划和完成定义
...
- 后续若引入 Sprint Contract，应优先把 contract 文件与对应 Planner artifact 互相链接
