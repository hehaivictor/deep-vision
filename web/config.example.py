...
REPORT_V3_MIN_REVIEW_ROUNDS = 0               # 报告最少审稿轮次：0=按档位默认(balanced=1, quality=2)
REPORT_V3_QUALITY_FORCE_SINGLE_LANE = True    # quality 档强制草案+审稿同一 lane，减少风格漂移
REPORT_V3_QUALITY_PRIMARY_LANE = "report"     # quality 档默认主 lane：report/question
REPORT_V3_RENDER_MERMAID_FROM_DATA = True     # 报告图表按结构化数据后端渲染，弱化模型自由绘图波动
REPORT_V3_WEAK_BINDING_ENABLED = True         # 允许对 risks/actions/open_questions 进行保守证据弱绑定
REPORT_V3_WEAK_BINDING_MIN_SCORE = 0.46       # 弱绑定最小相关性分数（越高越保守）
REPORT_V3_SALVAGE_ON_QUALITY_GATE_FAILURE = True  # V3 因门禁失败时是否尝试规则化挽救一次
REPORT_V3_FAILOVER_ENABLED = True             # V3 主网关失败后是否切备用网关再试一次
REPORT_V3_FAILOVER_LANE = "question"          # 备用网关 lane: question/report
REPORT_V3_FAILOVER_ON_SINGLE_ISSUE = True     # 审稿/门禁仅 1 条可修复 issue 时，允许切备用网关再试
REPORT_V3_BLINDSPOT_ACTION_REQUIRED_BALANCED = False  # balanced 档盲区默认先进入 open_questions
REPORT_V3_BLINDSPOT_ACTION_REQUIRED_QUALITY = True    # quality 档盲区默认要求补一条行动项
REPORT_V3_UNKNOWNS_TO_OPEN_QUESTIONS_ENABLED = True   # unknown 比例过高时自动补采 open_questions
REPORT_V3_UNKNOWNS_TO_OPEN_QUESTIONS_MAX_ITEMS = 3    # 自动补采的 unknown 条目上限
REPORT_V3_UNKNOWN_RATIO_TRIGGER = 0.65                # 触发 unknown 自动补采的比例阈值
