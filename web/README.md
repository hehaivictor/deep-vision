...
- 登录会话默认有效期为 `30` 天（过期后会自动回到登录页）

### 数据隔离规则

- 所有会话数据按 `owner_user_id` 归属，仅拥有者可见/可改/可删。
- 报告列表与报告详情按用户归属过滤，不可跨账号读取。
...
- **生成访谈报告**: ~5000 tokens/次
- **一次完整访谈**: 约 15-20 次问题生成 + 1 次报告 = ~20,000 tokens
